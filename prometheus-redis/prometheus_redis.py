...
sys.path.append(path.dirname(path.dirname(path.abspath('prometheus_redis.py'))))

from common import prometheus_poller,constants


class PrometheusRedisStats(prometheus_poller.PrometheusStats):
    """
    class PrometheusRedisStats
    """
    ...
    def __init__(self, cfg, port):
        ...
        :param port:
        """
        super(PrometheusRedisStats, self).__init__(cfg, port)
        self.poll_count = 0
        self.host = None
        self.port = None
        self.status = {}
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=LOG_FILE, level=logging.DEBUG, filemode='a',
                        ...
                        datefmt='%d/%b/%Y %H:%M:%S')
    OBJ = PrometheusRedisStats(CFG, PORT)
    OBJ.start()
